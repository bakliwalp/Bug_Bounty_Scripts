...
cat $subdomain_path/found.txt | grep $domain | sort -u | httpx  | grep https | sed 's/https\?:\/\///' | tee -a $subdomain_path/alive.txt

cat $subdomain_path/found.txt | grep $domain | sort -u | httpx -title -tech-detect -status-code >> $subdomain_path/techdetect.txt
